...
(function($){ 	
	...
	var createGrips = function(t){	
		...
		if(t.p && S && S[t.attr(ID)])memento(t,th);		//if 'postbackSafe' is enabled and there is data for the current table, its coloumn layout is restored
	...
	var memento = function(t, th){ 
		...
		if(th){										//in deserialization mode (after a postback)
			...
			if(t.opt.flush){ S[t.attr(ID)] =""; return;} 	//if flush is activated, stored data is removed
			w = S[t.attr(ID)].split(";");					//column widths is obtained
			tw = w[t.ln+1];
			...
			S[t.attr(ID)] ="";				//clean up previous data
			for(;i < t.c.length; i++){	//iterate through columns
				w = t.c[i].width();		//width is obtained
				S[t.attr(ID)] += w+";";		//width is appended to the sessionStorage object using ID as key
				m+=w;					//carriage is updated to obtain the full size used by columns
			}
			S[t.attr(ID)]+=m;							//the last item of the serialized string is the table's active area (width), 
												//to be able to obtain % width value of each columns while deserializing
			if(!t.f) S[t.attr(ID)] += ";"+t.width(); 	//if not fixed, table width is stored
		}	
	};
